...
def transcribe(model, video: Path, task: str):
...
def main():
    ...
    model = WhisperModel(args.model, device="cpu", compute_type="int8", download_root=str(ROOT / ".whisper_models"))
    ko_info, ko_segments = transcribe(model, video, "transcribe")
    ...
    en_info, en_segments = transcribe(model, video, "translate")
    ...
    print(f"english language={en_info.language} prob={en_info.language_probability:.3f} cues={len(en_segments)}", flush=True)
